...
def _build_memory_facts(memory_records: Sequence[MemoryRecord], display_name: str | None) -> list[str]:
    ...
    Формулировка усилена (2026-09-04, живая проверка на проде с дефолтной
    моделью `openai/gpt-4o-mini`): нейтральная фраза «Обращайся к
    пользователю по имени в каждом ответе» терялась среди остальных
    инструкций профиля (особенно у плотных персон вроде «Честный друг») —
    модель просто не обращалась по имени ни разу за диалог. Явная маркировка
    «обязательное правило, не просто факт» + «без исключений» — не
    гарантия для любой модели, но заметно надёжнее нейтральной формулировки.
    """
    facts = [record.text for record in memory_records if not is_display_name_fact(record)]
    if display_name is not None:
        facts.insert(
            0,
            f"Обязательное правило, не просто факт: пользователя зовут {display_name}. "
            "Обращайся к пользователю по имени в каждом ответе, без исключений.",
        )
    return facts
