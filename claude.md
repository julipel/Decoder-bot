...
**Внеспринтовая фича (2026-09-04, тем же днём): в `/model` виден `recommended_for` каталога.** Пользователь спросил, можно ли показывать краткое описание различий между моделями; предложено и согласовано — показать `AIModel.recommended_for` (поле каталога, существующее с Sprint 7, ADR-7.3, но нигде не отображавшееся) в подтверждении после выбора модели («Активна модель: X — хорошо подходит для: …»), не в самих кнопках клавиатуры (Telegram не даёт подзаголовок у inline-кнопки, только однострочный текст). `_format_model_selected_message()` в `handlers/model.py` — новый `MODEL_SELECTED_WITH_RECOMMENDATION_TEMPLATE` используется, только если `recommended_for` у модели не пуст, иначе прежний голый `MODEL_SELECTED_MESSAGE_TEMPLATE` (обратная совместимость с записями без этого поля). Ни `domain/model_catalog/`, ни `catalog.json` не менялись — только рендер уже существующих данных. 948 тестов (7 skipped без Qdrant).
