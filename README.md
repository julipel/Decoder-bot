...
- `/profile` — показывает каталог из 4 предустановленных профилей
  («Честный друг», «Психолог-наставник», «Личный ассистент» — профиль
  по умолчанию, «Контент-стратег») с inline-клавиатурой, текущий
  активный отмечен в тексте кнопки; выбор профиля меняет системную
  инструкцию, которую видит LLM при каждом следующем ответе —
  переключение не затрагивает уже отправленные сообщения и не влияет
